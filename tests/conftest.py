...
def invalid_data_errors():
    ...
    errors = {
        "required": [
            ("entity_missing_declarationSubject.json", "$[0]"),
            ("entity_recordDetails_missing_isComponent.json", "$[0].recordDetails")
        ],
        "enum": [
            ("entity_addressType_placeOfBirth.json", "$[0].recordDetails.addresses[0].type")
        ]
    }
    return errors
